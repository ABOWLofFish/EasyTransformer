...
# TODO: 7. ScaledDotProductAttention
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        # 然后关键词地方来了，下面这个就是用到了我们之前重点讲的attn_mask，把被mask的地方置为无限小，
        # softmax之后基本就是0，对q的单词不起作用
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        q_s = self.W_Q(Q).view(batch_size, -1, n_heads, d_k)\
            .transpose(1, 2)  # q_s: [batch_size x n_heads x len_q x d_k]
        k_s = self.W_K(K).view(batch_size, -1, n_heads, d_k)\
            .transpose(1, 2)  # k_s: [batch_size x n_heads x len_k x d_k]
        v_s = self.W_V(V).view(batch_size, -1, n_heads, d_v)\
            .transpose(1, 2)  # v_s: [batch_size x n_heads x len_k x d_v]

        # 输入进行的attn_mask形状是 batch_size x len_q x len_k，
        # 新的attn_mask : [batch_size x n_heads x len_q x len_k]，就是把pad信息重复了n个头上
        attn_mask = attn_mask.unsqueeze(1).repeat(1, n_heads, 1, 1)
        # 然后我们计算 ScaledDotProductAttention 这个函数，去7.看一下
        # 得到 context: [batch_size x n_heads x len_q x d_v], attn: [batch_size x n_heads x len_q x len_k]
        context, attn = ScaledDotProductAttention()(q_s, k_s, v_s, attn_mask)
        context = context.transpose(1, 2).contiguous()\
            .view(batch_size, -1, n_heads * d_v)  # context: [batch_size x len_q x n_heads * d_v]
        output = self.linear(context)
        return self.layer_norm(output + residual), attn  # output: [batch_size x len_q x d_model]


# TODO: 8. PoswiseFeedForwardNet
...
def get_attn_pad_mask(seq_q, seq_k):
    ...
    # 因为是分别对每个句子计算attn_mask，所以可以直接复制(expand)
    return pad_attn_mask.expand(batch_size, len_q, len_k)  # batch_size x len_q x len_k


# TODO: 3. PositionalEncoding 代码实现
...
# TODO: 5. EncoderLayer ：包含两个部分，多头注意力机制和前馈神经网络
...
# TODO: 2. Encoder 部分包含三个部分：词向量embedding，位置编码部分，注意力层及后续的前馈神经网络
...
# TODO: 1. 从整体网路结构来看，分为三个部分：编码层，解码层，输出层
class Transformer(nn.Module):
    ...
    def forward(self, enc_inputs, dec_inputs):
        ...
        # enc_output: [batch_size x src_seq_len x d_model]
        ...
        # dec_outputs做映射到词表大小, dec_output: [batch_size x tgt_seq_len x d_model]
